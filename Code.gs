...
// ================= KONFIGURASI SPREADSHEET ULASAN =================
// Spreadsheet TERPISAH tempat ulasan lama Anda sudah tersimpan.
const REVIEWS_SPREADSHEET_ID = '1cC163JYzdH-wSyX3F3ZFOah2-TFF_tnCjTN2TdSFL9c';
...
// ================= ULASAN (REVIEWS) — dari spreadsheet terpisah =================

function getReviewsSheet() {
  var ss = SpreadsheetApp.openById(REVIEWS_SPREADSHEET_ID);
  var sheet = ss.getSheetByName(REVIEWS_SHEET_NAME);
  if (!sheet) {
    throw new Error('Sheet "' + REVIEWS_SHEET_NAME + '" tidak ditemukan di spreadsheet ulasan.');
  }
  return sheet;
}
